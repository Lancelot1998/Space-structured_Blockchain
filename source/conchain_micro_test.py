...
from source.blockchain import Transaction, Block, Attachment, BlockData, LightBlockData, LightBlock, TransInput, \
    TransOutput, MacroBlockBody, MicroBlock, MacroBlockHeader
...
class PoWServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
    ...
    def make_macro_block_body(self, macro_block_header: MacroBlockHeader) -> 'MacroBlockBody':
        ...
        ref_hash = sorted(ref_hash, key=lambda micro_block: micro_block.timestamp)
        self.self_signed_micro_block.clear()

        macro_block_body = MacroBlockBody(hash_=macro_block_header.hash, ref_hash=ref_hash)
    ...
    def make_macro_block_header(self, nonce):
        ...
        parent_hash = [b'G\xfdk\x88\xda5\xff\x8c\x97t\x9f\xcb\xe0\xa8\x07S\x8b9t:.9\x1d\xee\xf4\xb1\xda\xd1r\xaf\xfcu']
